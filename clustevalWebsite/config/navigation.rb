...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |primary|
    ...
    #
      ...
      primary.item( :nav_programs, 'Methods', programs_path, :highlights_on => :subpath) do |sub_programs|
      ...
      primary.item( :nav_datasets, 'Dataset Configs', dataset_configs_path, :highlights_on => :subpath) do |sub_datasets|
        sub_datasets.item :nav_datasets_general, 'Overview', dataset_configs_path
        DatasetConfig.all(session).each do |dataset|
          sub_datasets.item( :nav_dataset, dataset.name, dataset_config_path(dataset)) do |sub_dataset|
            sub_dataset.item :nav_dataset_general, 'General', dataset_config_path(dataset)
            #sub_dataset.item :nav_dataset_statistics, 'Statistics', url_for(:controller => 'datasets', :action => 'show_statistics', :id => dataset.id)
            sub_dataset.item :nav_dataset_comparison, 'Best Qualities', url_for(:controller => 'dataset_configs', :action => 'show_comparison', :id => dataset.id)
            sub_dataset.item :nav_dataset_bestclusterings, 'Best Parameters', url_for(:controller => 'dataset_configs', :action => 'show_bestclusterings', :id => dataset.id)
            sub_dataset.item :nav_dataset_clusterings, 'All Matchings', url_for(:controller => 'dataset_configs', :action => 'show_clusterings', :id => dataset.id)
          end
        end
      end
    ...
    primary.item( :nav_submit, 'Submit', submits_path) do |sub_submit|
      ...
      primary.item( :nav_admins, 'Advanced', admins_path, :highlights_on => :subpath) do |sub_admin|
        sub_admin.item( :nav_program_configs, 'Method Configurations', program_configs_path) do |sub_program_configs|
