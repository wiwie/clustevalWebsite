class AddOrderToDataConfigsRankingsView < ActiveRecord::Migration
  def up
  	execute "drop materialized view run_results_data_configs_rankings"
  	execute "CREATE MATERIALIZED VIEW run_results_data_configs_rankings AS 
 SELECT run_results_parameter_optimizations_parameter_set_iterations.id AS t0_r0,
   ...
   FROM run_results_parameter_optimizations_parameter_set_iterations
     ...
     LEFT JOIN program_configs ON program_configs.id = run_results_parameter_optimizations.program_config_id
     LEFT JOIN data_configs as data_configs_bla ON data_configs_bla.id = run_results_parameter_optimizations.data_config_id
     LEFT JOIN data_configs ON data_configs.id = data_configs_bla.data_config_id
