...
ColVis.prototype = {
	...
	"_fnConstruct": function ( init )
	{
		...
		$(this.s.dt.oInstance).bind( 'column-reorder', function ( e, oSettings, oReorder ) {
			for ( i=0, iLen=that.s.aiInclude.length ; i<iLen ; i++ ) {
				that.s.aiInclude[i] = oReorder.aiInvertMapping[ that.s.aiInclude[i] ];
			}
	...
	"_fnAddButtons": function ()
	{
		...
		if ( $.inArray( 'all', this.s.aiInclude ) === -1 ) {
			for ( var i=0, iLen=columns.length ; i<iLen ; i++ )
			{
				if ( $.inArray( i, this.s.aiInclude ) !== -1 )
				{
	...
	"_fnDomShowAllButton": function ()
	{
		...
		return $(
			...
			.click( function (e) {
				for ( var i=0, iLen=that.s.abOriginal.length ; i<iLen ; i++ )
				{
					if (that.s.aiInclude.indexOf(i) === -1)
					{
